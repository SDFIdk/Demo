...
#
# Update        13-11-2014:
#               Det er tidsstemplet pÃ¥ filen MatrikelDownload.log der sammenlignes med vÃ¦rdien
...
# Update        03-12-2014:
# Name:         Nyt navn: Get_MAPINFO_Matr_zip_from_Atomfeed.py
#               Oprindeligt udviklet til GML formattet. Denne version et tilpasset til at hente matrikeldata i MapInfo formattet.
...
def hent_matr_zipfiler(ejerlav,Outputfolder, lastupdate):
    try:
        ...
        ftp.cwd('atomfeeds/MATRIKELKORT/ATOM/MAPINFO')
        tree=ET.parse(urllib.urlopen("http://download.kortforsyningen.dk/sites/default/files/feeds/MATRIKELKORT_MAPINFO.xml"))
        ...
        for node in entries:
             ...
             Ejerlavnr = zipurl[zipurl.find("MAPINFO")+8:len(zipurl)-26]
             if Ejerlavnr in ejerlav:
                if dato  > lastupdate:
                   filename = Ejerlavnr + "_MAPINFO_UTM32-EUREF89.zip"
