...
Ejerlav = ['1121459', '1121059', '1121057', '1121453']
...
def hent_matr_zipfiler(ejerlav,Outputfolder, lastupdate):
    try:
        ...
        for node in entries:
             ...
             Ejerlavnr = zipurl[zipurl.find("GML")+4:len(zipurl)-4]
             if Ejerlavnr in ejerlav:
                if dato  > lastupdate:
                   filename = Ejerlavnr + ".zip"
                   file = open(Outputfolder + os.sep + filename, 'wb')
                   ftp.retrbinary('RETR %s' % filename, file.write)
                   skriv_log(logfil, "donwloader ejerlav : " + Ejerlavnr + " d. " + str(dato))
                   file.close
        ftp.close
    except:
        tb = sys.exc_info()[2]
